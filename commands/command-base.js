...
const runCommand = async (message, client, distube) => {
  ...
  const mainCommand = name.replace(prefix, '');
  if (name.startsWith(prefix)) {
    const command = allCommands[mainCommand];
    if (!command) return;
...
module.exports.listen = (client, distube) => {
  // Listen for messages
  client.on('message', async (message) => {
    runCommand(message, client, distube)
  })
}
